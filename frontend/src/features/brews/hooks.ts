...
export interface BrewTaste {
  id: string; brew_id: string; score: number | null;
  ...
  notes: string | null; flavor_tags: { id: string; name: string }[];
  created_at: string; updated_at: string;
}

export interface Brew {
  id: string; bag_id: string; brew_setup_id: string; person_id: string;
  grind_setting: number | null; grind_setting_display: string | null;
  temperature: number | null; pressure: number | null; flow_rate: number | null;
  dose: number; yield_amount: number | null;
  pre_infusion_time: number | null; total_time: number | null;
  stop_mode_id: string | null; is_failed: boolean; notes: string | null;
...
interface BrewListParams extends PaginationParams {
  bag_id?: string; bean_id?: string; brew_setup_id?: string;
  person_id?: string; brewed_after?: string; brewed_before?: string;
}

export function useBrews(params: BrewListParams) {
  return useQuery<{ items: BrewListItem[]; total: number }>({
    queryKey: ['brews', params],
    queryFn: async () => { const { data } = await apiClient.get('/brews', { params }); return data; },
  });
}
...
export function useBrew(id: string) {
  return useQuery<Brew>({
    queryKey: ['brews', id],
    queryFn: async () => { const { data } = await apiClient.get(`/brews/${id}`); return data; },
    enabled: !!id,
  });
}
...
export function useCreateBrew() {
  ...
  return useMutation({
    mutationFn: async (body: Record<string, unknown>) => { const { data } = await apiClient.post('/brews', body); return data as Brew; },
    onSuccess: () => {
...
export function useUpdateBrew() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, ...body }: { id: string; [key: string]: unknown }) => { const { data } = await apiClient.patch(`/brews/${id}`, body); return data; },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['brews'] });
      qc.invalidateQueries({ queryKey: ['stats', 'brews'] });
      qc.invalidateQueries({ queryKey: ['stats', 'taste'] });
...
export function useDeleteBrew() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (id: string) => { await apiClient.delete(`/brews/${id}`); },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['brews'] });
      qc.invalidateQueries({ queryKey: ['stats', 'brews'] });
      qc.invalidateQueries({ queryKey: ['stats', 'taste'] });
...
export function useUpsertBrewTaste(brewId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (body: Record<string, unknown>) => { const { data } = await apiClient.put(`/brews/${brewId}/taste`, body); return data; },
    onSuccess: () => {
...
export function useDeleteBrewTaste(brewId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async () => { await apiClient.delete(`/brews/${brewId}/taste`); },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['brews', brewId] });
      qc.invalidateQueries({ queryKey: ['stats', 'taste'] });
    },
