...
import { useCreateBag, useUpdateBag, type Bag } from '../hooks';
...
function CreateInlineForm({
  endpoint, label, initialName, onCreated, onCancel,
}: {
  endpoint: string;
  label: string;
  initialName: string;
...
export default function BagFormDialog({ open, onClose, beanId, bag, onRetire, onActivate }: BagFormDialogProps) {
  const isEdit = !!bag;
  ...
  useEffect(() => {
    if (bag) {
      ...
      setStorageType(bag.storage_type_id ? { id: bag.storage_type_id, name: '' } : null);
    } else {
      setWeight('');
      setPrice('');
      setRoastDate('');
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        <Stack spacing={2} sx={{ pt: 1 }}>
          <TextField
            label="Weight (g)" type="number" value={weight}
          ...
          <TextField
            label="Roast Date" type="date" value={roastDate}
            onChange={(e) => setRoastDate(e.target.value)}
            slotProps={{ inputLabel: { shrink: true } }}
          />

          <TextField
            label="Opened At" type="date" value={openedAt}
            onChange={(e) => setOpenedAt(e.target.value)}
            slotProps={{ inputLabel: { shrink: true } }}
          />

          <TextField
            label="Bought At" type="date" value={boughtAt}
            onChange={(e) => setBoughtAt(e.target.value)}
            slotProps={{ inputLabel: { shrink: true } }}
          />

          <TextField
            label="Best Before" type="date" value={bestDate}
          ...
          <AutocompleteCreate<OptionItem>
            ...
            queryKey={['vendors']}
            fetchFn={async (q) => {
              const { data } = await apiClient.get('/vendors', { params: { q, limit: 50 } });
              return data;
            }}
            value={vendor}
          ...
          <AutocompleteCreate<OptionItem>
            ...
            fetchFn={async (q) => {
              const { data } = await apiClient.get('/storage-types', { params: { q, limit: 50 } });
              return data;
            }}
            value={storageType}
            onChange={(v) => setStorageType(v as OptionItem | null)}
            renderCreateForm={(props) => (
              <CreateInlineForm endpoint="storage-types" label="Storage Type" {...props} />
            )}
          />

          <FormControlLabel
            control={<Switch checked={isPreground} onChange={(_, c) => setIsPreground(c)} />}
            label="Pre-ground"
          />

          <TextField
            label="Frozen At" type="datetime-local" value={frozenAt}
            onChange={(e) => setFrozenAt(e.target.value)}
            slotProps={{ inputLabel: { shrink: true } }}
          />

          <TextField
            label="Thawed At" type="datetime-local" value={thawedAt}
