...
import {
  AppBar, Box, Drawer, Fab, IconButton, List, ListItemButton, ListItemIcon,
  ListItemText, ListSubheader, Toolbar, Tooltip, Typography, useMediaQuery, useTheme,
} from '@mui/material';
import {
...
export default function AppLayout() {
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh' }}>
      ...
      {isMobile ? (
        ...
        <Drawer variant="permanent"
          sx={{ width: drawerWidth, flexShrink: 0,
            '& .MuiDrawer-paper': { width: drawerWidth, transition: theme.transitions.create('width', { duration: 200 }), overflowX: 'hidden' } }}>
          {drawerContent}
        </Drawer>
      )}
      ...
      <Fab color="primary" variant="extended" onClick={() => navigate('/brews/new')}
